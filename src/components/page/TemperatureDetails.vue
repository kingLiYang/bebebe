          ...
          <table>
            <tr>
              <td colspan="6" style="background: #f0f0f0;color: rgb(255, 255, 255);">
                <span
                  style="float: left;margin-left: 10px;font-family: cursive;font-weight: 700;color:deepskyblue"
                >温度概况</span>
                <span
                  style="float: right;margin-right: 20px;font-family: cursive;font-weight: 700;color:deepskyblue"
                >设备编号：{{SheBeiHao}}</span>
              </td>
            </tr>
            <tr>
  ...
  methods: {
      ...
      this.$axios
        ...
        .then(res => {
          if (res.data.code == 0) {
            ...
            this.Tem_L = res.data.Tem_L || "0";
            this.Police_L = res.data.Police_L || "0";
    ...
    excelDetails() {
      window.location.href =
        "http://www.zjcoldcloud.com/berry/public/index.php/Init_way_bill/details?BillNumber=" +
        this.BillNumber +
        "&SheBeiHao=" +
        this.SheBeiHao +
        "&state=excel";
