        ...
        <div>
          ...
          <el-form :model="send" label-width="100px" :inline="true" style="padding:10px 0 0 0;">
            <el-form-item label="发货人">
              <el-select v-model="send.username" filterable placeholder="请选择" @change="handleSelect">
                <el-option
        ...
        <div>
          ...
          <el-form :model="receive" label-width="100px" :inline="true" style="padding:10px 0 0 0;">
            <el-form-item label="收货人">
              <el-select v-model="receive.username" filterable placeholder="请选择" @change='handleSelectreceive'>
                <el-option
  ...
  methods: {
      ...
      this.$axios
        ...
        .then(res => {
          ...
          this.drug_data.forEach(item => {
            ...
            item.isShow = true;
            item.drug_list.forEach(ite =>{
              ite.num = item.art_num;
            })
          });
    ...
    handleSelect(id) {
      ...
      this.sendName.forEach((item, index) => {
        if (item.id == id) {
          ...
          this.send.username = item.username;
          this.send.phone = item.phone;
          this.send.company = item.company;
          this.send.province = item.province;
          this.send.city = item.city;
          this.send.district = item.district;
    ...
    handleSelectreceive(id) {
      this.receiveName.forEach((item,index)=>{
        if(item.id == id){
      this.get_id = item.id;
      this.receive.username = item.username;
      this.receive.phone = item.phone;
      this.receive.company = item.company;
      this.receive.province = item.province;
      this.receive.city = item.city;
      this.receive.district = item.district;
